...
import { ChevronRight, History, LogOut, MapPin, Mic, Plus, Trash2, Users } from '@lucide/vue'
...
<template>
  <AppScreen>
    ...
    <div v-if="turnOffModalCopy" class="modal-overlay" @click.self="cancelTurnOff">
      <div class="modal" role="alertdialog" aria-modal="true" :aria-label="turnOffModalCopy.title">
        <h2>{{ turnOffModalCopy.title }}</h2>
        <p>{{ turnOffModalCopy.description }}</p>
    ...
    <div v-if="isAddGuardianOpen" class="modal-overlay" @click.self="closeAddGuardian">
      <div class="modal" role="dialog" aria-modal="true" aria-label="가족 추가">
        <h2>가족을 추가해 주세요</h2>
        <p>수상한 전화가 감지되면 이 분께 바로 알려드려요.</p>
    ...
    <div v-if="pendingDeleteGuardian" class="modal-overlay" @click.self="cancelDeleteGuardian">
      <div class="modal" role="alertdialog" aria-modal="true" :aria-label="`${pendingDeleteGuardian.name} 삭제`">
        <h2>{{ pendingDeleteGuardian.name }}님을 삭제할까요?</h2>
        <p>삭제하면 수상한 전화가 감지돼도 이 분께는 더 이상 알려드리지 않아요.</p>
